...
This store lives in the skill's `memory/` folder on disk, and `scripts/memory.py` writes to it directly — a memory change survives as soon as it's made; there is nothing to rebuild or reinstall. Batch related writes within a session for tidiness. If code execution is unavailable, you cannot run `memory.py`; tell the user and offer the updated memory as text (`render`).
